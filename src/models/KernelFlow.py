...
def _safe_inv(mat: tf.Tensor, 
              base_jitter: float = 1e-6, 
              max_tries: int = 8) -> tf.Tensor:
    dtype = mat.dtype
...
def compute_localized_B(x: tf.Tensor, 
                        prior_cov_override: Optional[tf.Tensor], 
                        localization: Optional[tf.Tensor], 
                        jitter: float):
...
def scalar_kernel_mahalanobis(x: tf.Tensor, 
                              B_inv: tf.Tensor, 
...
def compute_grad_logp(x_new: tf.Tensor, 
                      y: tf.Tensor, 
                      H: Optional[tf.Tensor], 
                      h_func: Optional[callable], 
                      R_inv: tf.Tensor, 
                      m: tf.Tensor, 
                      B_inv: tf.Tensor) -> tf.Tensor:
...
class KernelScalarFlow:
    ...
    def flow(self, 
             ...
             R: tf.Tensor,  
             H: Optional[tf.Tensor], 
             prior_cov_override: Optional[tf.Tensor] = None
        ...
        x_new = tf.identity(x)
        # Clear diagnostics for this flow evaluation
        self.flow_disp_norm_history = []
        self.jacobian_cond_history = []
        ...
        for _ in range(n_steps):
            ...
            x_new = x_new + delta
            # diagnostics: average displacement norm and a proxy Jacobian condition
            avg_disp = tf.reduce_mean(tf.norm(delta, axis=1))
            # proxy Jacobian: I + delta_s * B (B acts as a linear prefactor)
            J_proxy = tf.eye(tf.shape(B)[0], dtype=tf.float64) + delta_s * tf.cast(B, tf.float64)
            svals = tf.linalg.svd(J_proxy, compute_uv=False)
            cond_eps = tf.cast(1e-12, tf.float64)
    ...
    def filter(self, 
               ...
               weights=None,
               verbose: bool = False):
        """Sequentially apply the scalar kernel flow to `particles` for each observation.

        Args:
            observations: array-like or tf.Tensor of shape (T, obs_dim)
            particles: initial particle cloud, shape (N, state_dim)
            H: linear observation matrix or None when using `h_func`
            R: observation covariance
            h_func: optional nonlinear observation function
            prior_cov_override: optional prior covariance to pass to `flow`
            weights: optional initial particle weights (1D array length N). If None, uniform weights used.

        Returns:
            means_tf: tf.Tensor shape (T, state_dim)
            covs_tf: tf.Tensor shape (T, state_dim, state_dim)
            ll_tf: tf.Tensor shape (T,) approximate per-step log-likelihoods
        """
        obs_arr = np.asarray(observations)
        particles = tf.cast(tf.convert_to_tensor(particles), tf.float64)
        N = int(particles.shape[0])
        d = int(particles.shape[1])
        ...
        # pre-build obs distribution helper per-step
        for y in obs_arr:
            x_new, _ = self.flow(particles, y, h_func, R, H,  prior_cov_override=prior_cov_override)
            particles = x_new

            # weighted mean & covariance
            w_tf = tf.reshape(tf.convert_to_tensor(w, dtype=tf.float64), [-1, 1])
            mean = tf.reduce_sum(particles * w_tf, axis=0)
            ...
            # approximate marginal log-likelihood from particle predictive
            R_tf = tf.cast(tf.convert_to_tensor(R), tf.float64)
            obs_tril = tf.linalg.cholesky(R_tf + tf.eye(tf.shape(R_tf)[0], dtype=tf.float64) * 1e-12)
            obs_dist = tfd.MultivariateNormalTriL(loc=tf.zeros(tf.shape(R_tf)[0], dtype=tf.float64), scale_tril=obs_tril)

            logps = []
            for i in range(N):
                xi = tf.reshape(particles[i], [d])
                if h_func is not None:
                    hz = tf.reshape(tf.cast(h_func(xi), tf.float64), [-1])
                else:
                    hz = tf.reshape(tf.matmul(tf.cast(H, tf.float64), tf.reshape(xi, [-1, 1])), [-1])
                lp = obs_dist.log_prob(tf.reshape(tf.convert_to_tensor(y, dtype=tf.float64) - hz, [-1]))
                logps.append(float(lp.numpy()))

            logps_tf = tf.convert_to_tensor(np.asarray(logps), dtype=tf.float64)
            logw_tf = tf.math.log(tf.convert_to_tensor(w, dtype=tf.float64) + 1e-300)
            loglik = float(tf.reduce_logsumexp(logw_tf + logps_tf).numpy())
            lls.append(loglik)
            if verbose:
                try:
                    ll_val = float(loglik)
                ...
                print(f"KernelScalarFlow.filter step={len(lls)} mean={np.round(mean.numpy(),3)} loglik={ll_val}")

        means_tf = tf.convert_to_tensor(np.vstack(means), dtype=tf.float64)
        covs_tf = tf.convert_to_tensor(np.stack(covs), dtype=tf.float64)
        ll_tf = tf.convert_to_tensor(np.asarray(lls), dtype=tf.float64)
...
class KernelMatrixFlow:
    ...
    def flow(self, 
             ...
             R: tf.Tensor,  
             H: Optional[tf.Tensor], 
             prior_cov_override: Optional[tf.Tensor] = None) -> Tuple[tf.Tensor, tf.Tensor]:
        ...
        for _ in range(n_steps):
            # Compute posterior gradient at current particle locations
            grad_logp = compute_grad_logp(x_new, y, H, h_func, R_inv, m, B_inv)
            # grad_logp: (N, d)  -> per-particle posterior gradient

            # Build the vector field f and update particles as in scalar flow
            # f: (N, d) accumulator for the vector field
            f = tf.zeros_like(x_new, dtype=tf.float64)
            for dd in range(int(d.numpy())):
                # xi_d: (N,1)  d-th component column for all particles
                xi_d = tf.reshape(x_new[:, dd], [-1, 1])
                # xj_d: (1,N)  transpose of xi_d for pairwise differences
                xj_d = tf.transpose(xi_d)
                # diff: (N,N) where entry (i,j) = x_i^{(d)} - x_j^{(d)}
                diff = xi_d - xj_d
                # B_dd: scalar  the (dd,dd) element of prior covariance B
                B_dd = tf.cast(B[dd, dd], tf.float64)
                # denom: scalar = 2 * alpha_val * B_dd
                denom = 2.0 * alpha_val * B_dd
                # K: (N,N)  component-wise kernel matrix for dimension dd
                K = tf.exp(- tf.square(diff) / denom)
                # dK: (N,N)  derivative of K w.r.t. particle positions
                dK = - diff / (alpha_val * B_dd) * K
                # grad_j_d: (1,N)  row vector of gradients for component dd
                grad_j_d = tf.reshape(grad_logp[:, dd], [1, -1])
                # term: (N,)  per-particle scalar after summing over j
                term = tf.reduce_sum(K * grad_j_d, axis=1) + tf.reduce_sum(dK, axis=1)
                # construct a (N,d) update with the computed column at position dd
                f = f + tf.concat([
                    tf.zeros([N_int, dd], dtype=tf.float64),
                    tf.reshape(term / tf.cast(N, tf.float64), [N_int,1]),
                    tf.zeros([N_int, int(d.numpy())-dd-1], dtype=tf.float64)
                ], axis=1)
            # delta: (N,d) = f @ B^T * delta_s
            delta = tf.matmul(f, tf.transpose(B)) * delta_s
            # x_new: (N,d)
            x_new = x_new + delta
            # diagnostics: average displacement and proxy Jacobian condition
            avg_disp = tf.reduce_mean(tf.norm(delta, axis=1))
            J_proxy = tf.eye(tf.shape(B)[0], dtype=tf.float64) + delta_s * tf.cast(B, tf.float64)
            svals = tf.linalg.svd(J_proxy, compute_uv=False)
            cond_eps = tf.cast(1e-12, tf.float64)
    ...
    def filter(self, 
               ...
               weights=None,
               verbose: bool = False):
        """Sequentially apply the matrix-kernel flow to `particles` for each observation.

        Signature mirrors `KernelScalarFlow.filter`.
        """
        obs_arr = np.asarray(observations)
        particles = tf.cast(tf.convert_to_tensor(particles), tf.float64)
        N = int(particles.shape[0])
        ...
        for y in obs_arr:
            x_new, _ = self.flow(particles, 
                                 y, 
                                 h_func, 
                                 R, 
                                 H, 
                                 prior_cov_override=prior_cov_override)
            particles = x_new
            ...
            if verbose:
                try:
                    ...
                    ll_val = float(loglik.numpy()) if hasattr(loglik, 'numpy') else loglik
                print(f"KernelMatrixFlow.filter step={len(lls)} mean={np.round(mean.numpy(),3)} loglik={ll_val}")
...
if __name__ == "__main__":
    ...
    # Quick single-step sanity flows (keep existing diagnostic)
    x_s, log_s = ks.flow(particles, tf.zeros(d, dtype=tf.float64), lambda x: x, R, H)
    x_m, log_m = km.flow(particles, tf.zeros(d, dtype=tf.float64), lambda x: x, R, H)

    print("KernelFlow single-step diagnostics")
    ...
    # Generate a short sequence of T observations and test `filter` methods
    T = 5
    true_states = []
    observations = []
    x_t = tf.zeros(d, dtype=tf.float64)
    # small process noise for test
    Q = tf.constant(np.eye(d) * 0.05, dtype=tf.float64)
    for _ in range(T):
    ...
    print("\nTesting filter methods with T=", T)
    ks_means, ks_covs, ks_ll = ks.filter(obs_all.numpy(), particles.numpy(), lambda x: x, R, H)
    km_means, km_covs, km_ll = km.filter(obs_all.numpy(), particles.numpy(), lambda x: x, R, H)

    print("KernelScalarFlow filter final mean:", np.round(ks_means[-1].numpy(), 3))
    print("KernelMatrixFlow filter final mean:", np.round(km_means[-1].numpy(), 3))

    moved_ks = not np.allclose(particles.numpy(), ks_means[0].numpy())
    moved_km = not np.allclose(particles.numpy(), km_means[0].numpy())
    if moved_ks and moved_km:
        print("KERNELFLOW FILTER TEST: PASS — both filters produced non-trivial means")
        sys.exit(0)
    else:
        print("KERNELFLOW FILTER TEST: FAIL — one or both filters produced trivial output")
        sys.exit(2)
