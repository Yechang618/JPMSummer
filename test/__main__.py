...
def check_dependencies():
    ...
    missing = []
    for pkg in ["tensorflow", "tensorflow_probability"]:
        if find_spec(pkg) is None:
            missing.append(pkg)
    
    if missing:
        print("Missing required packages:", ", ".join(missing))
        print("Please install with: python -m pip install " + " ".join(missing))
        sys.exit(2)
...
def main() -> None:
	...
	try:
		...
		print("Failed to import KalmanFilter. Make sure the package is installed:")
